    ...
    if($_POST['action']=="submit")
    {
        ...
        if(count($users)>=1)
        {
			if (password_verify($_POST['password'], $users['password']))
			{
				...
				{
					$hour = time() + 3600;
					setcookie('nightify', $users['password'], $hour);
					$year = time() + 31536000;
					setcookie('nightify_remember_me', $_POST['email'], $year);
				}
				else
				{
					setcookie('nightify_remember_me', $_POST['email'], 1);
				}
			}
			else
			{
...
if(isset($_SESSION['userid'])||isset($_COOKIE['nightify'])){
	if($_POST['location'] != '')
...
<br><br>
<a href="/"><img src="https://i.imgur.com/w1tTQQJ.png" width="100%" alt="Pelto"></a>
<br><br><br><br>
<div class="row">
<div class="col-xs-1">
...
<label for="inputEmail" class="sr-only">Email address</label>
 <input type="email" id="inputEmail" name="email" class="form-control" placeholder="Email address" value="<? if(isset($_COOKIE['nightify_remember_me'])) echo $_COOKIE['nightify_remember_me']; else echo $_POST['email'];?>" required autofocus>
</div>
</div>
<div class="row">
<div class="col-xs-1">
...
<div class="checkbox">
<label>
<input type="checkbox" name="remember-me" value="remember-me" <? if(isset($_COOKIE['nightify_remember_me'])) echo 'checked';?>> Remember me
</label>
</div>
<button class="btn btn-lg btn-success btn-block" type="submit" name="action" value="submit">Sign in</button>
<? echo $message; ?>
<br><br>
<center>
<b>Don't have an account? <a href="signup.php" style="color: #228B22;">Sign Up</a></b>
<br><br>
